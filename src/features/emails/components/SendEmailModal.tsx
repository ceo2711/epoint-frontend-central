...
interface SendEmailModalProps {
  ...
  basePath: string;
  onClose: () => void;
  /** Hilo + composer (clientes). Prospectos siguen en modo redactar. */
  threadMode?: boolean;
  onThreadOpened?: () => void;
}
...
export function SendEmailModal({
  ...
  const [view, setView] = useState<"compose" | "history">(threadMode ? "history" : "compose");
    ...
    <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
      ...
      <div className="flex justify-end gap-3">
        <Button
          type="button"
          variant="secondary"
          onClick={threadMode ? () => setComposing(false) : onClose}
          disabled={sending}
        >
          {t("common.cancel")}
        </Button>
  ...
  return (
    <Modal
      title={threadMode ? t("emailCompose.threadTitle") : view === "compose" ? t("emailCompose.title") : t("emailCompose.history")}
      ...
      headerActions={
        threadMode ? (
          <button
            type="button"
            className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-brand transition hover:bg-brand-muted"
            aria-label={t("emailCompose.composeAction")}
            title={t("emailCompose.composeAction")}
    ...
    >
      ...
      {threadMode && composing ? (
        <Modal
          title={t("emailCompose.title")}
          subtitle={`${recipientName} · ${recipientEmail}`}
