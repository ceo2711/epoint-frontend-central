...
export default function PortalHomePage() {
  ...
  const { data: client, isPending } = usePortalMe(token);
  ...
  if (isPending) {
    return (
      <>
        ...
        <PageContent>
          <PortalPageLoader label={t("portal.loading")} />
        </PageContent>
      </>
    );
  }
...
function StepCard({
  ...
  const missingBlock =
    incomplete && missingItems.length > 0 ? (
      <div className="step-missing mt-3 rounded-lg px-3 py-2">
        <p className="step-missing-title text-xs font-semibold">{missingTitle}</p>
        <ul className="step-missing-list mt-1 list-disc space-y-0.5 pl-4 text-xs leading-snug">
  ...
  const badgeClass = incomplete
    ? "bg-[var(--accent-gold)] text-[var(--brown-950)] shadow-[0_4px_10px_rgba(196,168,130,0.35)]"
    : complete
      ? "step-index"
      : "bg-blue-600 shadow-blue-600/25";
  const ctaClass = incomplete ? "step-missing-cta" : complete ? "step-cta" : "text-blue-600";
