...
export interface NavItem {
  ...
  /** Producto requerido para entrar (portal cliente). */
  entitlement?: "credit" | "course" | "mentorship";
}
...
export function clientEntitlements(user: User | null): {
  credit: boolean;
  course: boolean;
  mentorship: boolean;
} {
  if (!user?.entitlements) {
    return { credit: true, course: false, mentorship: false };
  }
...
export function getAccessibleNavItems(
  ...
  if (user.role.code === "CLIENT") {
    ...
    return clientNav.map((item) => {
      const needs = item.entitlement;
      const entitled = !needs || flags[needs];
      const boardLocked = item.href === "/portal/tablero" && !unlocked;
      if (!entitled || boardLocked) {
        return {
          ...item,
          disabled: true,
          disabledHintKey:
            item.href === "/portal/tablero" && entitled && boardLocked
              ? "nav.boardLocked"
              : "nav.productLocked",
        };
      }
